 ...
 * 
 */
public class DegreesToRadiansFunctionTests {

    private DegreesToRadiansFunction op = null;
	...
	op = new DegreesToRadiansFunction(null);
    }
	...
	try {
	    op.setArguments(args);
	    assertEquals(Math.toRadians(0),op.resolve().toDouble(), 
	...
	try {
	    op.setArguments(args);
	    assertEquals(Math.toRadians(-180),op.resolve().toDouble(), 
		    FuzzyEquals.TOLERANCE);
	} catch (InvalidArgumentsException e) {
	    e.printStackTrace();
	    fail();
