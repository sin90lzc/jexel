...
public class MinOperatorTests extends TestCase {

	private MinOperator op = null;
	...
	public void testResolve5() {
		...
		try {
			op.setArguments(args);
			assertEquals(op.resolve(null).toDouble(), 0.27d);
		} catch (InvalidArgumentsException e) {
			e.printStackTrace();
			fail();
		} catch (ArgumentCastException e) {
	...
	public void testResolve6() {
		...
		try {
			op.setArguments(args);
			assertEquals(op.resolve(null).toDouble(), -0.11d);
		} catch (InvalidArgumentsException e) {
			e.printStackTrace();
			fail();
		} catch (ArgumentCastException e) {
